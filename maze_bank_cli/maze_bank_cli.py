...
"""
    ...
    while True:
        ...
        if user_input in user_options:
            ...
            print(
                "Invalid Input. Enter (l) to Log In to existing account or (s) to Sign Up for a new account.\n"
            )
...
def login():
    ...
    while True:
        ...
        if valid_account:
            account_number = get_value_from_csv(
                "clients.csv", "username", username, "accountNumber"
            )
...
def signup():
    ...
    while True:
        ...
        while not is_valid_phone_number(phone_number):
            print("Invalid Input. Phone Number should be 11 digits long.")
            phone_number = input("Enter your Phone Number: ").strip()
        # Email
        email = input("Enter your email address: ").strip().lower()
        while not is_valid_email(email):
            print(
                "Invalid input. Make sure you are using the valid email address format."
            )
            email = input("Enter your email address: ").strip().lower()
        # Account Type
        account_type = (
        ...
        while not is_valid_username(username):
            print(
                "Invalid Input. Username can only contain alphabets, numbers and/or an underscore."
            )
...
def find_client_logins(file_name, username, password):
    ...
    with open(file_name, "r") as csvfile:
        ...
        rows.append(header)
        # Get index of relevant columns to match it with relevant inputs
        username_index = header.index("username")
        password_index = header.index("password")
        for row in csv_reader:
            if row[username_index] == username and row[password_index] == password:
                is_valid_username = True
                is_valid_password = True
                break
            elif row[username_index] == username and row[password_index] != password:
                is_valid_username = True
                break
            elif row[username_index] != username and row[password_index] == password:
                is_valid_password = True
    ...
    if is_valid_username and is_valid_password:
        return True
    elif not is_valid_username and is_valid_password:
        print("Incorrect Username or Username does not exist.")
        return False
    elif is_valid_username and not is_valid_password:
...
def deposit(account_number):
    ...
    while True:
        try:
            ...
            value = get_value_from_csv(
                "accounts.csv", "accountNumber", account_number, "accountBalance"
            )
            ...
            while not is_completed:
                ...
                if user_input in user_options:
                    if user_input == "y" and is_valid_account(recipients_account):
                        ...
                        sender_balance = get_value_from_csv(
                            "accounts.csv",
                            "accountNumber",
                            account_number,
                            "accountBalance",
                        )
                        new_sender_balance = float(sender_balance) - float(
                            formatted_transfer_amount
                        )
                        ...
                        recipient_balance = get_value_from_csv(
                            "accounts.csv",
                            "accountNumber",
                            str(recipients_account),
                            "accountBalance",
                        )
                        new_recipient_balance = float(recipient_balance) + float(
                            formatted_transfer_amount
                        )
                    ...
                    print(
                        "Invalid Input. Enter (y) for yes to confirm or (n) for no to cancel.\n"
                    )
...
def generate_account_number():
    ...
    while True:
        if is_available("accounts.csv", str(account_number), "accountNumber"):
...
def change_value_in_csv(
    filename, condition_column, condition_value, target_column_name, new_value
):
    rows = []
    # Open csv file and read its contents
...
def is_valid_username(username):
    ...
    return match(pattern, username) is not None


# Function checks if password meets requirements
def is_valid_password(password):
    pattern = r"^^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$"
    return match(pattern, password) is not None


# Function checks if the phone number only contains numbers and is 10 digits long
def is_valid_phone_number(phone_number):
    pattern = r"^[0-9]{,11}$"
    return match(pattern, phone_number) is not None


# Function stores data of new sign up into relevant csv files
...
# Function checks if value is already taken
def is_available(filename, value, column):
    with open(filename, "r") as csvfile:
        csv_reader = csv.DictReader(csvfile)
        for row in csv_reader:
            if row[column] == value:
